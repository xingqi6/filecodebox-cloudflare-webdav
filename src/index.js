...
<body>
    ...
    <script>
        ...
        // 文件上传函数（已移除分片功能，直接上传）
        async function uploadFile(file, expireValue, expireStyle) {
            // 直接使用原有上传方式
        ...
        document.getElementById('fileForm').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                // 使用直接上传
                const result = await uploadFile(currentFileData, expireValue, expireStyle);
...
app.post('/api/share/file', async (c) => {
  try {
    ...
    const maxFileSize = (function(){
      ...
      if (!n) return 90 * 1024 * 1024; // 默认90MB
      return n < 100000 ? n * 1024 * 1024 : n;
    })();
